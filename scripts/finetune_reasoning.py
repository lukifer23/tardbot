...
def load_reasoning_data(data_config: DataConfig) -> list:
    processed_dir = Path(data_config.processed_data_dir)
    processed_file = processed_dir / "reasoning_data.jsonl"
    logger = get_logger(__name__)
    logger.info("Loading reasoning/CoT data from %s", processed_file)
    data = load_jsonl_dataset(processed_file, "Reasoning")
    logger.info("Loaded %d reasoning examples", len(data))
    return data
...
parser.add_argument("--dataset-limit", type=int, default=None, help="Optional limit on reasoning examples")
    ...
    data = load_reasoning_data(data_config)
    ...
    train_dataset = ReasoningDataset(train_data, tokenizer, max_length=model_config.max_position_embeddings)
    val_dataset = ReasoningDataset(val_data, tokenizer, max_length=model_config.max_position_embeddings)
